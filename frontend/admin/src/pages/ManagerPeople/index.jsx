...
function ManagerPeople() {
	...
	const paginatedPeople = getPeople.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage);
		...
		if (loading) {
			...
			return paginatedPeople.map((people) => (
				<tr key={people.id}>
					...
					<td>
						<Button variant="danger" onClick={() => handleDeleteClick(people.id)}>
							Xóa
						</Button>
					</td>
				</tr>
			...
			<div className="pagination-controls">
				<div className="pagination-buttons">
					{Array.from({ length: totalPages }, (_, index) => index + 1).map((page) => (
						<Button key={page} variant="outline-primary" onClick={() => handlePageChange(page)} active={page === currentPage} style={{ margin: "0 5px" }}>
							{page}
						</Button>
					))}
				</div>
