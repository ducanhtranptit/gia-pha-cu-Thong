...
const CreatePersonModal = ({ show, fetchData, handleClose }) => {
	...
	const handleFileUpload = async (file) => {
		...
		try {
			const response = await axios.post("http://localhost:2504/api/v1/core/upload", formData, {
				headers: {
			...
			<Modal.Body>
				<Form onSubmit={handleSubmit}>
					...
					<div {...getRootProps({ className: "dropzone" })} className="dropzone">
						<input {...getInputProps()} />
						{fileUrl ? <img src={fileUrl} alt="Preview" className="preview-image" /> : <Button variant="btn btn-outline-info">Thêm ảnh đại diện</Button>}
					</div>
					...
					{formData.showSpouseForm && (
						<>
							...
							<div {...getSpouseRootProps({ className: "dropzone" })} className="dropzone">
								...
								{spouseFileUrl ? <img src={spouseFileUrl} alt="Preview" className="preview-image" /> : <Button variant="btn btn-outline-info">Thêm ảnh đại diện</Button>}
							</div>
							<div className="form-row">
								<Form.Group className="form-group">
